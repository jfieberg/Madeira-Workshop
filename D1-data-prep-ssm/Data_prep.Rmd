...
We can also use functions from the `amt` package to get some of useful information information. Here, we can get summary info about the sampling rate.

```{r amt.check, attr.source = ".numberLines", collapse=TRUE}
tracks_gps_amt <- tracks_gps_O %>% mk_track(.x = x, .y = y, .t = time, id=ID)
tracks_gps_amt %>% summarize_sampling_rate()
```
...
```{r crawl_30_min_gps_plot, attr.source = ".numberLines", message=FALSE, warning= FALSE, collapse=TRUE}
...
```{r crawl_extract, attr.source = ".numberLines", message=FALSE, warning= FALSE, collapse=TRUE}
...
You could alternatively use `amt` to split the tracks into "burst" (i.e., segment) and regularize at the same time.

```{r amt_reg, attr.source = ".numberLines"}
tracks_gps_30 <- tracks_gps_amt %>% nest(data = -"id") %>% 
  mutate(steps = map(data, function(x) 
    x %>% track_resample(rate = minutes(30), tolerance = minutes(5))))
tracks_gps_30 <- tracks_gps_30 %>% unnest(steps)
head(tracks_gps_30, 20)
```

YOu would then need to place NAs afterward.
...
As for te narwhal data, we can use the `amt` package function `summarize_sampling_rate()` to get some summary statistics.

```{r .gentoo.amt.check, attr.source = ".numberLines", collapse=TRUE}
gentoo_amt <- gentoo_tracks %>% mk_track(.x = Longitude, .y = Latitude, .t = time, id=ID)
gentoo_amt %>% summarize_sampling_rate(time_unit = "hour")
```
...
The function `fit_ssm` applies some prefiltering before applying the state-space model for data. One of the prefiltering arguments is `vmax`, which set the maximum travel rate (m/s). Here, we set it to 50 km/hr, which is large so as not to remove too many locations. To see the default values for things like maximum travel rate and min allowable time difference, look at the help file (`?fit_ssm`).

Importantly, unlike `crawl`, which using an analytically solution for the continuous time movement model, `animotum` uses an approximation. In essence, we choose a discrete time interval at which we want to use to make our approximation of continuous time. The smaller this time interval the more accurate the approximation is. In essence, we want to set the argument `time.step` to a small value. Here we choose 1 hour, which is may not be fine scale enough, but the smaller the time interval the longer it will take to fit the model. Thus, to be able to run the code during the tutorial, we chose a coarser time interval
...
Here, the model converged for all tracks. Convergence can be hard to get at times, and you may need to play with the `time.step`.

The package also provides easy plotting functions. When we use `type = 1`, we can see the latitude and longitude separately. When we use `type = 2` we see the tracks. If we want to use the estimated location at the time of the observation, you want to specify the argument `what` to "fitted".
...
gentoo_mpi_t <- grab(gentoo_mpi, what = "fitted")
head(gentoo_mpi_t)
```
...
For some downstream analyses, we may need regularise tracks. As we saw above, it can be hard to find the perfect time interval, and it will be dependent on the question and analysis you'll do afterward. When we fitted the model above, we using a small time interval (30 min) to help the approximation, but this is much finer than the information we have and it would  be better to subsample at a coarser scale closer to the scale of our analysis. Here we will use 4 hour. 

First, let us explore the raw regular track with points at the time scale we used when fitting the model. To be able to extract that information, we simply set the argument `what` to "predicted". This will return predicted locations at times where we don't necessarily have observations. 

```{r plot_mpr, attr.source = ".numberLines"}
plot(gentoo_mpi, what ="predicted", type=1, ask=FALSE)
plot(gentoo_mpi, what ="predicted", type=2, ask=FALSE)
```

Map the tracks.

```{r map_mpr, attr.source = ".numberLines"}
aniMotum::map(gentoo_mpi, what = "predicted", normalise = TRUE, group = TRUE)
```

We can see that we get much more points than our observations and that when there are data gaps, the predicted tracks are likely much straighter than they the real track of the gentoo penguins.

Here, we will grab the predicted tracks and subsample them for other analyses. If you don't use the argument `what = "predicted"`, grab will return the fitted data at irregular time intervals.

```{r grab_mpr, attr.source = ".numberLines"}
gentoo_mpr_t <- grab(gentoo_mpi, what = "predicted")
head(gentoo_mpr_t)
```

Notice that the locations are now at every 1 hour. If we want them every 4 hours, we simply take one row every 4.

```{r subsample_mpr, attr.source = ".numberLines"}
gentoo_mpr_4hr <- gentoo_mpr_t[seq(1, nrow(gentoo_mpr_t), by=4), ]
```

We can now plot the subsampled data.

```{r plot_mpr_4hr, attr.source = ".numberLines"}
ggplot() +
  geom_spatial_path(data =  gentoo_mpr_4hr, 
                    aes(x = lon, y = lat, lty = id), 
                    crs = 4326) +
  geom_spatial_point(data =  gentoo_mpr_4hr, 
                    aes(x = lon, y = lat, col = g), 
                    crs = 4326) +
  coord_sf(crs = 4326)
```
...
set.seed(2025)
psim <- sim_post(gentoo_mpi, what = "predicted", reps = 4)
