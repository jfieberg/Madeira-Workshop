...
```{r}
gentoo_tracks <- read.csv(here("D1-data-prep-ssm", "data", "gentoo_Argos.csv"))
head(gentoo_tracks)
gentoo_tracks %>% count(Platform.ID.No.)
gentoo_5 <- gentoo_tracks %>% 
  subset(Platform.ID.No. %in% c(170895, 170902, 170905, 170914, 170936)) %>% 
  dplyr::select(Platform.ID.No., 
                Loc..date, Longitude, Latitude, Loc..quality) %>% 
  mutate(ID = factor(Platform.ID.No.),
    datetime = ymd_hms(Loc..date),
    Qual = Loc..quality) %>%
  arrange(Platform.ID.No., datetime) %>%
...
gentoo_5 <- gentoo_5 %>% filter(!is.na(Longitude) & !is.na(Latitude)) 
sum(is.na(gentoo_5))
gentoo_5_sf <- gentoo_5 %>%
