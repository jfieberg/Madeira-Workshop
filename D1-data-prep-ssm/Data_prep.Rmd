...
We see that the most frequent time gap is $0$ min, followed by $2$, $1$, and $3$ min. But we also see a large variability. 
...
gentoo_ani <- gentoo_tracks %>%
  rename(id = ID, date = time, lc = Qual,
...
```{r}
gentoo_mpi <- fit_ssm(gentoo_ani, model = "mp")
head(gentoo_mpi)
summary(gentoo_mpi)
```
...
```{r}
plot(gentoo_mpi, what ="fitted", type=2, ncol=2, ask = FALSE)
```
