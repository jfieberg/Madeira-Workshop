...
pbRes <- RTMB::oneStepPredict(mPbTmb, 
                              observation.name = "y", 
                        data.term.indicator = "keep", 
                        discrete=FALSE, trace=FALSE)
head(pbRes, 30)
tail(pbRes, 30)

plot(observation ~ mean, data=pbRes)
plot(observation ~ residual, data=pbRes)
plot(residual, data=pbRes)
pbRes <- pbRes %>% mutate(res = observation - mean)
```
...
pbResLon <- pbRes[seq(3, nrow(pbRes)/2),
                  "res"]
pbResLat <- pbRes[seq(nrow(pbRes)/2 + 3, nrow(pbRes)),
                  "res"]
```
